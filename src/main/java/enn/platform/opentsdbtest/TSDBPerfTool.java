...
//  private static AtomicLong LOOP = new AtomicLong(getTimestamp("2000-01-01 00:00:00") / 1000);
  ...
  private void initConf() {
    this.tagNumber = getConf("TAG_NUMBER", 8);
    this.tps = getConf("TPS", 50000);
    this.batchNumber = getConf("BATCH_NUMBER", 400);
    this.threads = getConf("THREADS", 8);
    this.port = getConf("PORT", 4242);
    this.address = getConf("ADDRESS", "10.39.42.5");
...
//    this.writeUrl =
//        "http://" + this.address + ":" + this.port + "/api/put/" + (this.sync ? "?sync&" : "?")
//            + "sync_timeout=" + this.SYNC_TIMEOUT_MS;
